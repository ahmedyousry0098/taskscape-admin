...
export default function Dashboard() {
  ...
  const { getAllEmployees, getScrums, EmployeeLoading, ScrumLoading } = useAppSelector((state => state.allEmployees))
  const { getAllProjects, isLoading } = useAppSelector((state => state.allProjects))
      ...
      <div className="flex flex-wrap justify-center items-center mx-auto w-full mt-4 text-white">
        <div className="flex flex-col justify-between bg-sky-950 w-56 mx-5 h-24 rounded-2xl p-3 mb-4 ">
          ...
          {EmployeeLoading && ScrumLoading ?
            ...
            <p key={1} className='text-center text-2xl text-amber-500'>
              {getAllEmployees?.employees?.length + getScrums?.scrums?.length}</p>
          }
        </div>
        ...
        <div className="flex flex-col justify-between bg-sky-950 w-56 mx-5 h-24 rounded-2xl p-3 mb-4 ">
          ...
          {isLoading ?
            <p className='text-center text-2xl text-amber-500'>Loading...</p>
            :
            <p key={2} className='text-center text-2xl text-amber-500'>{getAllProjects?.projects?.length}</p>
          }
        </div>

        <div className="flex flex-col justify-between bg-sky-950 w-56 mx-5 h-24 rounded-2xl p-3 mb-4 ">
          <h1 className='px-3 text-base'>Performance:</h1>
          <p key={3} className='text-center text-2xl text-amber-500'>{getAllProjects?.projects?.length} * 100 / 2 </p>
        </div>
      ...
      <div>
        <div className="flex flex-wrap justify-around mx-auto w-full items-base mt-4 text-sky-950">
          <div className="w-[450px] mx-3 shadow-lg rounded-2xl px-3 py-1 mb-4">
            <EmpType getAllEmployees={getAllEmployees} getScrums={getScrums}
              EmployeeLoading={EmployeeLoading} ScrumLoading={ScrumLoading} />
          </div>

          <div className="w-[650px] mx-3 shadow-lg rounded-2xl px-3 py-1 mb-4">
            <Experience getAllEmployees={getAllEmployees} getScrums={getScrums}
              EmployeeLoading={EmployeeLoading} ScrumLoading={ScrumLoading} />
          </div>
        </div>
